...
function WeightCalc({ product }: { product: Product }) {
  const [meters, setMeters] = useState<string>("6");
  const [tons, setTons] = useState<string>("");
  const [mode, setMode] = useState<"length" | "weight">("length");

  const wpm = product.weightPerMeter ?? 1;
  const weightResult = (parseFloat(meters) || 0) * wpm;
  const lengthResult = ((parseFloat(tons) || 0) * 1000) / wpm;
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="flex gap-1 bg-gray-100 rounded-full p-1 w-fit">
        <button
          ...
          className={`px-4 py-1.5 rounded-full text-[13px] font-semibold transition-all ${mode === "length" ? "bg-white shadow-sm text-gray-900" : "text-gray-500 hover:text-gray-700"}`}
        >
          Длина → Вес
        </button>
        <button
          onClick={() => setMode("weight")}
          className={`px-4 py-1.5 rounded-full text-[13px] font-semibold transition-all ${mode === "weight" ? "bg-white shadow-sm text-gray-900" : "text-gray-500 hover:text-gray-700"}`}
        >
          Вес → Длина
        </button>
      </div>
      ...
      {mode === "length" ? (
        <div className="flex flex-col gap-3">
          <label className="text-[13px] font-semibold text-gray-700">Длина, м</label>
          <input
            type="number"
            value={meters}
            min="0"
            ...
            className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-[15px] font-medium focus:outline-none focus:border-brand-primary transition-colors"
          />
          <div className="flex items-center justify-between bg-blue-50 border border-blue-100 rounded-xl px-4 py-3">
            <span className="text-[13px] text-gray-600 font-medium">Масса</span>
            <span className="text-[18px] font-bold text-brand-primary">
              {weightResult >= 1000
                ? `${(weightResult / 1000).toFixed(3)} т`
                : `${weightResult.toFixed(2)} кг`}
            </span>
          </div>
        </div>
      ) : (
        <div className="flex flex-col gap-3">
          <label className="text-[13px] font-semibold text-gray-700">Масса, т</label>
          <input
            type="number"
            value={tons}
            min="0"
          ...
          />
          <div className="flex items-center justify-between bg-blue-50 border border-blue-100 rounded-xl px-4 py-3">
            <span className="text-[13px] text-gray-600 font-medium">Длина</span>
            <span className="text-[18px] font-bold text-brand-primary">
              {isFinite(lengthResult) && (parseFloat(tons) || 0) > 0
                ? `${lengthResult.toFixed(1)} м`
                : "—"}
            </span>
          </div>
        </div>
      ...
      <p className="text-[11px] text-gray-400">
        Теоретическая масса: {wpm} кг/м. Расчёт по ГОСТ, погрешность ±3–5%.
      </p>
    </div>
  );
}
      ...
      <div className="border-b border-gray-100 bg-white">
        <div className="container py-4">
          <nav className="flex items-center gap-1.5 text-[12px] font-semibold text-gray-400 flex-wrap">
            ...
            <ChevronRight />
            <Link href={`/catalog?category=${encodeURIComponent(product.category)}`} className="hover:text-gray-700 transition-colors">
              {product.category}
            </Link>
            <ChevronRight />
        ...
        <div className="container mt-6">
          <div className="border border-gray-200 rounded-3xl overflow-hidden">
            <div className="px-6 sm:px-8 py-5 border-b border-gray-100 bg-gray-50">
              <p className="text-[16px] font-bold text-gray-900">Расчёт партии</p>
              <p className="text-[13px] text-gray-500 mt-0.5">Теоретическая масса по ГОСТ</p>
            </div>
            <div className="px-6 sm:px-8 py-6">
              <WeightCalc product={product} />
            </div>
          </div>
        </div>
      )}
