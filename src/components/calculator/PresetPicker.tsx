...
import { CALCULATOR_PRESET_GROUPS, CALCULATOR_PRESET_MAP, CALCULATOR_PRESETS } from "@/lib/metal-calculator/config/presets";
...
export const PresetPicker = ({ value, onChange, compact = false }: PresetPickerProps) => {
  ...
  return (
    <div className={`flex flex-col ${compact ? "gap-1.5" : "gap-3"}`}>
      <label className={labelClass}>1. Готовая позиция</label>
      <select
        value={value ?? ""}
        onChange={(event) => onChange(event.target.value || null)}
        className={selectClass}
      >
        <option value="">Выберите из {CALCULATOR_PRESETS.length} позиций</option>
