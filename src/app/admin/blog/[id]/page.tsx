...
export default function EditBlogPostPage() {
  ...
  useEffect(() => {
    fetch(`/api/admin/blog/${id}`).then(r => r.json()).then(setForm);
  }, [id]);
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-4 max-w-5xl">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-[18px] font-bold text-slate-900">Статья #{id}</h1>
        ...
        <div className="flex items-center gap-2">
          <a href={`/blog/${form.slug}`} target="_blank"
            className="inline-flex items-center gap-1.5 text-slate-500 hover:text-slate-700 text-[13px] font-medium px-3 py-2 rounded-xl border border-slate-200 hover:bg-slate-50 transition-colors">
            ...
            {!preview && (
              <div className="flex items-center gap-1 flex-wrap">
                {[
                  { label: "H2", insert: "\n## " },
                  { label: "H3", insert: "\n### " },
                  { label: "B", insert: "****", cursor: 2, bold: true },
                  { label: "—", insert: "\n\n---\n\n" },
                  { label: "• Список", insert: "\n- " },
